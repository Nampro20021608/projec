<?php
    ...
    if($_SERVER['REQUEST_METHOD'] == 'POST'){
        ...
        if (isFormValidated()){
            ...
            $product['ten_sp'] = $_POST['ten_san_pham'];
...
<body>
    ...
    <form action="<?php echo $_SERVER['PHP_SELF'] ; ?>" method="POST">
        ...
        <input type="hidden" name="ma_san_pham" id="ma_san_pham" value="<?php echo isFormValidated()? $san_pham['ma_san_pham']: $_POST['ma_san_pham'] ?>" min="1">
        <br>

        <label for="ten_san_pham">Tên Sản Phẩm:</label> 
        <input type="text" name="ten_san_pham" id="ten_san_pham" value="<?php echo isFormValidated()? $san_pham['ten_sp']: $_POST['ten_san_pham']; ?>">
        <br>
        ...
        <input type="text" name="gia" id="gia" value="<?php echo isFormValidated()? $san_pham['gia']: $_POST['gia'] ?>">
        <br>

        <label for="soluong">Số lượng:</label>
        <input type="number" name="soluong" id="soluong" value="<?php echo isFormValidated()? $san_pham['soluong']: $_POST['soluong'] ?>" min="1">
        <br>

        <label for="hinh_anh">Hình ảnh:</label>
        <input type="text" name="hinh_anh" id="hinh_anh" value="<?php echo isFormValidated()? $san_pham['hinh_anh']: $_POST['hinh_anh'] ?>">
        <br>

        <label for="thuoc_tinh">Thuộc tính:</label><br>
        <textarea name="thuoc_tinh" id="thuoc_tinh" cols="100" rows="3"><?php echo isFormValidated()? $san_pham['thuoc_tinh']: $_POST['thuoc_tinh'] ?></textarea>
        <br>

        <label for="xuat_su">Xuất sứ:</label>
        <input type="text" name="xuat_su" id="xuat_su" value="<?php echo isFormValidated()? $san_pham['xuat_su']: $_POST['xuat_su'] ?>">
        <br>
